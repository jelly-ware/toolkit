...
public class Enumeration {
	...
	public static <T extends Enum<T>, V> T valueOf(Class<T> enumType,
			String fieldName, V value) {
		...
		if (enumMap == null) {
			throw new IllegalStateException(
					"Enum " + enumType + " is not initialized");
		}
		Map<?, Enum<?>> fieldMap = enumMap.get(fieldName);
		if (enumMap == null) {
			throw new IllegalArgumentException("Field " + fieldName
					+ " is not defined in Enum " + enumType);
		}
	...
	private static <T> Object getFieldValue(Class<?> enumType, T enumElem,
			String fieldName) {
		Throwable t = null;
		try {
